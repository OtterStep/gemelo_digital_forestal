...
El flujo se orquesta desde App.tsx:
...
En la pestaña Escenario, el usuario ajusta sliders en ScenarioControls:
...
Al presionar "Ejecutar simulación", el frontend llama a POST /simular de api.py. El backend ejecuta esta cadena:
...
El backend devuelve todo un objeto Simulation (tipado en types.ts): baseline, scenario, delta, temporal, trees, insumos, terreno, metadata_parcela.

🖥️ 4 Vistas / Pestañas
1️⃣ Escenario (Scenario.tsx)
...
Traza del modelo (ModelTrace.tsx): muestra cada predictor con valor/unidad, resultado y notas interpretativas
...
2️⃣ Mapas 2D/3D (Maps.tsx)
Mapa 2D (ForestMap2D.tsx):
...
3️⃣ Serie Temporal (Timeline.tsx)
Gráfico (TimeSeries.tsx) de 12 meses con estacionalidad aplicada
4 líneas: biomasa base, biomasa escenario, riesgo base, riesgo escenario
Tabla detallada con valores mensuales
4️⃣ Exportar Reporte (Export.tsx)
...
Endpoint POST /explicar (api.py:471-481):

Compone un prompt estructurado (_prompt_explicacion) en español con:
...
Llama a la API REST de Google Gemini (generativelanguage.googleapis.com)
Devuelve el texto explicativo al frontend
...
En api.ts, cada llamada fetch tiene un catch que:
